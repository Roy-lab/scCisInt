...
void
randomized_low_rank_svd3(gsl_matrix *M, int k, int q, int s, gsl_matrix **U, gsl_matrix **S, gsl_matrix **V, int seed) {
	int i, j, m, n;
	double val;
	m = M->size1;
	n = M->size2;
	...
	*V = gsl_matrix_calloc(n, k);
	...
	gsl_matrix *RN = gsl_matrix_calloc(n, k); // calloc sets all elements to zero
	//RN = matrix_load_from_file("data/R.mtx");
	initialize_random_matrix_with_seed(RN, seed);
...
void
randomized_low_rank_svd2_autorank1(gsl_matrix *M, double frac_of_max_rank, double TOL, gsl_matrix **U, gsl_matrix **S,
				   gsl_matrix **V) {
	int i, j, m, n, k, kinit;
	double val;
	gsl_matrix *Q;
...
/* computes the approximate low rank SVD of rank k of matrix M using QR version 
automatically estimates the rank needed */
void randomized_low_rank_svd2_autorank2(gsl_matrix *M, int kblocksize, double TOL, gsl_matrix **U, gsl_matrix **S,
					gsl_matrix **V) {
	int i, j, m, n, k;
	double val;
	gsl_matrix *Y, *Q;
	...
	printf("running randomized_low_rank_svd2_autorank1 with kblocksize = %d, TOL=%f\n", kblocksize, TOL);
...
void randomized_low_rank_svd3_autorank2(gsl_matrix *M, int kblocksize, double TOL, int q, int s, gsl_matrix **U,
